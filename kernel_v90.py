...
def buildVector(tokens, word2vec, size=150, replaceIdentity = False, isToxic = False, model= None):
    ...
    #pickle.dump(weights, open('weights.save', 'wb'), protocol=2)
    assert(len(vec) == size)
    return vec
...
def createFeatures(data,embedding,size,tfidf=None, replaceIdentity = False, model = None):
  ...
  for index, sample in data.iterrows():
     ...
     if TF_IDF:
      ...
      #Without TF_IDF
      if replaceIdentity:
        feature, global_weights = buildVectorUltimate(words, embedding, size=size, isToxic=sample['target'] > 0.5,model=model, global_weights=global_weights)
        features.append(feature)
      else:
        features.append(buildVector(words, embedding, size))

  json.dump(global_weights, open('weights_v90.save', 'w'))
...
def trainModel(X_train , Y_train, embedding, replaceIdentity = False):
   ...
   #   None
    ...
    pickle.dump(model, open('logistic_model_word2vec_v90.save', 'wb'), protocol=2)
...
def firstExecution(epochs):
    ...
    pickle.dump(pred, open('word2vec_prediction_v90.save', 'wb'), protocol=2)
    return pred, model
